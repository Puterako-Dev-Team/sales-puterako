    ...
    <div class="bg-white rounded-lg shadow p-6">
        <div class="flex justify-between mb-4">
            ...
            <form method="POST" action="{{ route('notifications.readAll') }}">
                @csrf
                <button type="submit" class="text-sm px-3 py-2 text-green-600 rounded hover:text-green-700 border">Tandai sebagai sudah dibaca</button>
            </form>
        </div>
        @forelse (auth()->user()->notifications as $notif)
            <div class="flex items-start gap-4 p-4 mb-3 rounded {{ $notif->read_at ? 'bg-gray-50' : 'bg-orange-50' }}">
                <div class="flex-1">
