...
@section('content')
    ...
    <div class="container mx-auto p-8">
        ...
        <div class="bg-white shadow rounded-lg loading-overlay" id="tableContainer">
            ...
            <div id="tableContent">
                @include('tipe.table-content', ['tipes' => $tipes])
            </div>
        </div>
    ...
    <div id="formSlide" class="fixed inset-0 bg-black bg-opacity-30 z-50 hidden">
        <div class="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-lg transition-transform transform translate-x-full" id="formPanel">
            <div class="flex flex-col h-full">
                ...
                <!-- Form -->
                <form id="tipeForm" class="flex-1 flex flex-col">
                    @csrf
...
const ROUTES = {
    store: "{{ route('tipe.store') }}",
    filter: "{{ route('tipe.filter') }}",
    base: "{{ url('tipe') }}"
};
...
const tipeForm = document.getElementById('tipeForm');
...
function setupAdd(){
    resetForm();
    formTitle.textContent = 'Tambah Tipe';
    tipeForm.dataset.mode = 'add';
    tipeForm.action = ROUTES.store;
    openSlide();
}
...
function setupEdit(id){
    ...
    .then(d => {
        formTitle.textContent = 'Edit Tipe';
        tipeForm.dataset.mode = 'edit';
        tipeForm.action = `${ROUTES.base}/${id}`;
    ...
    .catch(e=>{
        window.notyf.error('Gagal memuat data tipe untuk diedit');
    });
}

/* ================== SUBMIT FORM (AJAX) ================== */
tipeForm.addEventListener('submit', function(e){
    e.preventDefault();
    const mode = tipeForm.dataset.mode || 'add';

    const fd = new FormData(tipeForm);
    if(mode === 'edit'){
        fd.append('_method', 'PUT');
    }

    const submitBtn = tipeForm.querySelector('button[type="submit"]');
    submitBtn.disabled = true;
    submitBtn.classList.add('opacity-70','cursor-not-allowed');

    fetch(tipeForm.action, {
        method: 'POST',
        headers: {'X-Requested-With': 'XMLHttpRequest'},
        body: fd
    })
    ...
    .then(res => {
        if(res.success){
            window.notyf.success(res.message || 'Tipe berhasil disimpan');
            closeSlide();
            performFilter();
        } else {
            window.notyf.error(res.message || 'Gagal menyimpan tipe');
        }
    })
    .catch(err=>{
        window.notyf.error('Gagal menyimpan tipe');
    })
    .finally(() => {
...
btnConfirmDelete.addEventListener('click', ()=>{
    ...
    .then(res=>{
        if(res.success){
            window.notyf.success(res.message || 'Tipe berhasil dihapus');
            closeConfirmDelete();
            performFilter();
        } else {
            window.notyf.error(res.message || 'Gagal menghapus tipe');
        }
    })
    .catch(()=>{
        window.notyf.error('Gagal menghapus tipe');
    });
});
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    const totalTipes = {{ $tipes->total() }};
    if(totalTipes > 0){
        resultsInfo.innerHTML = "Menampilkan {{ $tipes->firstItem() }} sampai {{ $tipes->lastItem() }} dari {{ $tipes->total() }} data";
    }
});
</script>
@endpush
